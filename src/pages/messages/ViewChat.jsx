...
import {doc,  addDoc, collection, deleteDoc, onSnapshot } from 'firebase/firestore';
...
const ViewChat = () => {
    ...
    const { users, messages, doctors, marriages, lawyers, calls, blocks, donates } = useData()
    ...
    // const myphoto = cuUser?.photo? cuUser?.photo : process.env.PUBLIC_URL + `${cuUser?.avatar}` || marry?.photo || dr?.photo || law?.photo
      ...
      // const chat = Object.entries(chats)?.find(c =>c[0] === id)
            ...
            <div className="chatroom_head">
                <div className="chat_head_left">
                    ...
                    <div className="chat_rec_photo">
                      {/* <img src={chat[1]?.userInfo.photo} alt="" />  */}
                    </div>
                    <div className="chat_body">
                        {/* <h4 className='chat_member_name'>{data.isUser?.name}</h4>                     */}
                    </div>
                </div>
